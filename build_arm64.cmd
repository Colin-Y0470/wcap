...
where /Q cl.exe || (
  ...
  call "!VS!\VC\Auxiliary\Build\vcvarsall.bat" arm64 || exit /b 1
)

if "%VSCMD_ARG_TGT_ARCH%" neq "arm64" (
  echo ERROR: please run this from MSVC arm64 native tools command prompt, 32-bit target is not supported!
  exit /b 1
)
...
cl.exe /nologo /W3 /WX wcap.c wcap.res /link /INCREMENTAL:NO /MANIFEST:EMBED /MANIFESTINPUT:wcap.manifest /SUBSYSTEM:WINDOWS /merge:_RDATA=.rdata || exit /b 1
del *.obj *.res >nul
